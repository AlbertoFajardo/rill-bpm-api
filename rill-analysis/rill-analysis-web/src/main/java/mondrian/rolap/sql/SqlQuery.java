...
public class SqlQuery {
        ...
        String alias)
    {
        ...
        // Add by MENGRAN at 2012-06-23
        Method m = ReflectionUtils.findMethod(dialect.getClass(), "setColumnAliases", Map.class);
        if (m != null) {
        	ReflectionUtils.invokeMethod(m, dialect, new Object[] {columnAliases});
        }
        // -------------------------END
        return alias;
    }
    ...
    private int matchIndex(String s, List<String> list) {
    	
    	for (int i =0; i < list.size(); i++) {
    		if (list.get(i).replaceAll("`", "").equals(s.replaceAll("`", ""))) {
    			return i;
    		}
    	}
    	
    	throw new IllegalArgumentException(s + " not exists in " + list);
    }
    // -------------------------END
    /**
        ...
        if (dialect.getClass().getName().equals("nu.com.rill.analysis.data.hive.HiveDialect")) {
        	if (from.size() > 1) {
	        	...
	        	for (String w : where) {
	        		...
	        		if (!discardFrom.contains(from.get(left))) {
	        			fc.add(from.get(left));
	        			discardFrom.add(from.get(left));
	        		}
	        		if (!discardFrom.contains(from.get(right))) {
	        			fc.add(from.get(right));
	        			discardFrom.add(from.get(right));
	        		}
	        		fc.toBuffer(buf, generateFormattedSql, prefix, fc.size() == 1 ? " JOIN " : "  ", " JOIN ", "", "");
	        		buf.append(" ON (" + w + ") ");
	        		discardWhere.add(w);
	        	}
	        	
	        	from.removeAll(discardFrom);
	        	where.removeAll(discardWhere);
        	}
        }
        // -------------------------END
        from.toBuffer(

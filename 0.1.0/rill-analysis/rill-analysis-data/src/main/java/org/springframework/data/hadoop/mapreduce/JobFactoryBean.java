...
public class JobFactoryBean implements InitializingBean, FactoryBean<JobConf>, BeanNameAware {
...
//	private Class<? extends RawComparator> groupingComparator;
	...
	public JobConf getObject() throws Exception {
		return job;
	}

	public Class<?> getObjectType() {
		return (job != null ? job.getClass() : JobConf.class);
	}
	...
	public void afterPropertiesSet() throws Exception {
		...
		ResourcePatternResolver rl = new HdfsResourceLoader(FileSystem.get(cfg));

		if (!CollectionUtils.isEmpty(inputPaths)) {
			for (String path : inputPaths) {
				FileInputFormat.addInputPath(cfg, validatePaths(path, rl, true));
			}
		}

		if (StringUtils.hasText(outputPath)) {
			FileOutputFormat.setOutputPath(cfg, validatePaths(outputPath, rl, false));
		}

		if (compressOutput != null) {
			FileOutputFormat.setCompressOutput(cfg, compressOutput);
		}

		if (codecClass != null) {
			FileOutputFormat.setOutputCompressorClass(cfg, codecClass);
		}
...
//			JobConf conf = (JobConf) job.getConfiguration();
			job.setJar(jar.getURI().toString());
		}

		processJob(job);
	}

	private void configureMapperTypesIfPossible(JobConf j, Class<? extends Mapper> mapper) {
		// Find mapper
	...
	private void configureReducerTypesIfPossible(JobConf j, Class<? extends Reducer> reducer) {
	...
	protected void processJob(JobConf job) throws Exception {
		// no-op
	}
